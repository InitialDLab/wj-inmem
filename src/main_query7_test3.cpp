...
void test_dynamic(date_t &cond_date1, date_t &cond_date2){
	...
	auto d = hash_join(c, vec_s_p, nullptr, &cond_func_supplier);
	double count = 0;
	for(auto &x:d){
		auto p = x;
		while(p->raw_tag != RAW_LINEITEM)
	...
	}
	{
		...
		ripple_4_ratio_test(table, a, key_func, cond_func, result_func, step_time, max_time, .95, true, total_size, 1);
	}
