...
#include <cstring>
static size_t max_time = 1000000 * 60 * 10;
...
void test_dynamic(){
	...
	}
	{
		...
		cond_func[1] = &cond_func_lineitem;
		...
		&cond_date1, &cond_date2,nullptr, key_func, result_func, step_time, max_time,
	...
	std::function<bool(base_raw *)> cond_func_supplier = [&](base_raw *input)->bool{\
	...
	node[2].cond_func = &cond_func_lineitem;
	//node[3].cond_func = &cond_func_supplier;
	...
	{
	timer mytimer;
	auto b = hash_join(vec_c_p, vec_o_p, &cond_func_customer, nullptr);
	auto c = hash_join(b, vec_l_p, nullptr, &cond_func_lineitem);
	auto d = hash_join(c, vec_s_p, nullptr, nullptr);
	double count = 0;
	for(auto &x:d){
	...
	}
	{
		...
		cond_func[0] = &cond_func_supplier;
		//cond_func[1] = &cond_func_customer;
		//cond_func[2] = &cond_func_orders;
		cond_func[1] = &cond_func_lineitem;
		func_type nation_customer = [&](base_raw *input)->uint32_t{\
		...
		ripple_4_ratio_test(table, a, key_func, cond_func, result_func, step_time, max_time, .95, true, total_size, value * 0.01, 1);
		uint32_t key = 4;
		index_ripple_query7(p3->data, p2->data, p1->data, p0->data, &key,
		&cond_date1, &cond_date2,nullptr, key_func, result_func, step_time, max_time,
		.95, value*0.01, true, 1);
	}
	}
