...
void test_dynamic(date_t &cond_date1, date_t &cond_date2){
	...
	}
	{
		...
		cond_func[3] = &cond_func_customer;
		...
		for(size_t i = 0; i < 100; ++i){
			...
			auto tmp = ripple_4_ratio(table, a, key_func, cond_func, result_func, value, 0.01*value, step_time, max_time, .95, false, 1);
			r_value += tmp.second;
			//std::cout<<"n: "<<i<<"\t"<<r_value/(i+1)<<std::endl;
		}
		std::cout<<"ripple_result: "<<r_value/100<<std::endl;
	}
